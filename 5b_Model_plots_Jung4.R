...
# same as other script but using Jung4

# plotting, percNH only - LU held at Cropland
# plotting interaction with UI, LU held at Cropland
...
# load in the models
# all biodiversity
load(paste0(datadir, "/Richness_Jung4_Tropical.rdata")) # sr1.trop
load(paste0(datadir, "/Richness_Jung4_Temperate.rdata")) # sr1.temp
load(paste0(datadir, "/Abundance_Jung4_Tropical.rdata")) # ab1.trop
load(paste0(datadir, "/Abundance_Jung4_Temperate.rdata")) # ab1.temp
...
pred_tab3 <- data.frame(percNH_Jung4_RS = median(sites.sub$percNH_Jung4_RS),
...
ggsave(filename = paste0(outdir, "/Rich_LUUI_Jung4.pdf"), width = 6, height = 3, uni = "in")
...
## tropical ##
pred_tab <- expand.grid(percNH_Jung4_RS = seq(from = min(sites.sub$percNH_Jung4_RS),
                                              to = max(sites.sub$percNH_Jung4_RS),
                                              length.out = 500),
...
pred_tab$percNH_Jung4 <- (pred_tab$percNH_Jung4_RS *scalers[1]) + scalers[2]

pred.sr <- PredictGLMERRandIter(model = sr1.trop$model,data = pred_tab)

pred.sr <- exp(pred.sr)

refRow <- which((pred_tab$Predominant_land_use=="Cropland") & (pred_tab$percNH_Jung4 == 0))
...
refRow <- which((pred_tab2$Predominant_land_use=="Cropland") & (pred_tab2$percNH_Jung4 == 0))
...
ggsave(filename = paste0(outdir, "/Plot_richness_percNH_Jung4.pdf"))
...
ggsave(filename = paste0(outdir, "/Plot_richness_percNH_LU_Jung4.pdf"))
...
refRow <- which((pred_tab$Use_intensity=="Minimal use") & (pred_tab$percNH_Jung4 == 0))
...
refRow <- which((pred_tab2$Use_intensity=="Minimal use") & (pred_tab2$percNH_Jung4 == 0))
...
ggsave(filename = paste0(outdir, "/Plot_richness_percNH_UI_Jung4.pdf"))
...
ggsave(filename = paste0(outdir, "/Abundance_LUUI_Jung4.pdf"), width = 6, height = 3, uni = "in")
...
ggsave(filename = paste0(outdir, "/Plot_Abundance_percNH_Jung4.pdf"))
...
ggsave(filename = paste0(outdir, "/Plot_Abundance_percNH_LU_Jung4.pdf"))
...
ggsave(filename = paste0(outdir, "/Plot_Abundance_percNH_UI_Jung4.pdf"))
...
# pollinators
load(paste0(datadir, "/Richness_Jung4_Tropical_POLLINATORS.rdata")) # sr1.trop
load(paste0(datadir, "/Richness_Jung4_Temperate_POLLINATORS.rdata")) # sr1.temp
load(paste0(datadir, "/Abundance_Jung4_Tropical_POLLINATORS.rdata")) # ab1.trop
load(paste0(datadir, "/Abundance_Jung4_Temperate_POLLINATORS.rdata")) # ab1.temp
...
# basic table of median values and reference factors
pred_tab3 <- data.frame(percNH_Jung4_RS = median(sites.sub.pols$percNH_Jung4_RS),
                        Use_intensity = "Minimal use",
                        Predominant_land_use = "Primary vegetation",
                        #Tropical = "Temperate",
                        Species_richness = 0,
...
ggsave(filename = paste0(outdir, "/Supp_Rich_LUUI_POLLINATORS_Jung4.pdf"), width = 6, height = 3, uni = "in")
...
## tropical ##
pred_tab <- expand.grid(percNH_Jung4_RS = seq(from = min(sites.sub.pols$percNH_Jung4_RS),
                                              to = max(sites.sub.pols$percNH_Jung4_RS),
                                              length.out = 500),
                        Predominant_land_use = factor(c("Cropland"),
...
ggsave(filename = paste0(outdir, "/Plot_richness_percNH_Jung4_POLLINATORS.pdf"))
...
ggsave(filename = paste0(outdir, "/Plot_richness_percNH_LU_Jung4_POLLINATORS.pdf"))
...
ggplot(data = plot_data) +
  geom_line(aes(x = percNH_Jung4, y = PredMedian, col = Use_intensity)) +
  geom_ribbon(aes(x = percNH_Jung4, ymin= PredLower, ymax = PredUpper, fill = Use_intensity), alpha = 0.3) +
  geom_rug(data = sites.sub.pols, aes(x = percNH_Jung4, col = Use_intensity), size = 0.1) +
  geom_hline(yintercept = 0, linetype = 'dashed') +
  facet_wrap(~realm) +
  ylim(c(-50, 100)) +
  xlim(c(0, 1)) +
...
ggsave(filename = paste0(outdir, "/Plot_richness_percNH_UI_Jung4_POLLINATORS.pdf"))
...
ggsave(filename = paste0(outdir, "/Abundance_LUUI_Jung4_POLLINATORS.pdf"), width = 6, height = 3, uni = "in")
...
ggsave(filename = paste0(outdir, "/Plot_Abundance_percNH_Jung4_POLLINATORS.pdf"))
...
ggsave(filename = paste0(outdir, "/Plot_Abundance_percNH_LU_Jung4_POLLINATORS.pdf"))
...
ggsave(filename = paste0(outdir, "/Plot_Abundance_percNH_UI_Jung4_POLLINATORS.pdf"))
